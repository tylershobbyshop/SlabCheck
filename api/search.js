const https = require('https');

const EBAY_TOKEN = process.env.EBAY_TOKEN || 'v^1.1#i^1#I^3#f^0#p^1#r^0#t^';

function detectCondition(title) {
  const t = (title || '').toLowerCase();
  if (t.includes('psa 10') || t.includes('psa10') || t.includes('gem mint')) return 'PSA 10';
  if (t.includes('psa 9.5')) return 'PSA 9.5';
  if (t.includes('psa 9')) return 'PSA 9';
  if (t.includes('bgs 9.5')) return 'BGS 9.5';
  if (t.includes('bgs 10')) return 'BGS 10';
  if (t.includes('cgc 10')) return 'CGC 10';
  if (t.includes('cgc 9.5')) return 'CGC 9.5';
  if (t.includes('sgc 10')) return 'SGC 10';
  if (t.includes('psa') || t.includes('bgs') || t.includes('cgc') || t.includes('sgc')) return 'Graded';
  return 'Raw';
}

// Get sold items using Buy Order API with sold filter
function fetchSold(query) {
  return new Promise((resolve, reject) => {
    const q = encodeURIComponent(query);
    // Use item_summary with AUCTION buyingOption to find completed auctions
    // and filter=buyingOptions to get recently sold
    const path = `/buy/marketplace_insights/v1_beta/item_sales/search?q=${q}&limit=50&sort=itemEndDate`;

    const options = {
      hostname: 'api.ebay.com',
      path,
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${EBAY_TOKEN}`,
        'X-EBAY-C-MARKETPLACE-ID': 'EBAY_US',
        'Accept': 'application/json',
        'User-Agent': 'SlabCheck/1.0',
      }
    };

    const req = https.request(options, (res) => {
      let data = '';
      res.on('data', c => data += c);
      res.on('end', () => {
        try {
          const json = JSON.parse(data);
          // marketplace_insights returns itemSales array
          const items = json.itemSales || json.items || [];
          if (!items.length) {
            // Fall back to browse API if no sold data
            return resolve(null);
          }
          const sales = items.map(item => ({
            title: item.title || '',
            price: parseFloat(item.lastSoldPrice?.value || item.price?.value || 0),
            date: (item.lastSoldDate || item.itemEndDate || '').split('T')[0],
            condition: item.condition || detectCondition(item.title),
            url: item.itemWebUrl || item.itemAffiliateWebUrl || '#',
          })).filter(s => s.price > 0 && s.date)
            .sort((a, b) => new Date(b.date) - new Date(a.date));
          resolve(sales);
        } catch(e) {
          resolve(null); // Fall through to browse API
        }
      });
    });
    req.on('error', () => resolve(null));
    req.end();
  });
}

// Fallback: Browse API for active listings (shows market prices)
function fetchBrowse(query) {
  return new Promise((resolve, reject) => {
    const q = encodeURIComponent(query);
    const path = `/buy/browse/v1/item_summary/search?q=${q}&filter=buyingOptions:{FIXED_PRICE|AUCTION}&sort=newlyListed&limit=50`;

    const options = {
      hostname: 'api.ebay.com',
      path,
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${EBAY_TOKEN}`,
        'X-EBAY-C-MARKETPLACE-ID': 'EBAY_US',
        'Accept': 'application/json',
        'User-Agent': 'SlabCheck/1.0',
      }
    };

    const req = https.request(options, (res) => {
      let data = '';
      res.on('data', c => data += c);
      res.on('end', () => {
        try {
          const json = JSON.parse(data);
          const items = json.itemSummaries || [];
          const sales = items.map(item => ({
            title: item.title || '',
            price: parseFloat(item.price?.value || 0),
            date: (item.itemEndDate || new Date().toISOString()).split('T')[0],
            condition: item.condition || detectCondition(item.title),
            url: item.itemWebUrl || '#',
          })).filter(s => s.price > 0)
            .sort((a, b) => new Date(b.date) - new Date(a.date));
          resolve(sales);
        } catch(e) {
          reject(new Error(e.message));
        }
      });
    });
    req.on('error', e => reject(e));
    req.end();
  });
}

module.exports = async (req, res) => {
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Cache-Control', 's-maxage=120'); // cache 2 mins

  const query = req.query.q;
  if (!query) return res.status(400).json({ error: 'Query required' });

  try {
    // Try sold data first
    const sold = await fetchSold(query);
    if (sold && sold.length > 0) {
      return res.json(sold);
    }
    // Fall back to browse (active listings = current market prices)
    const browse = await fetchBrowse(query);
    return res.json(browse);
  } catch(e) {
    return res.status(500).json({ error: e.message });
  }
};
